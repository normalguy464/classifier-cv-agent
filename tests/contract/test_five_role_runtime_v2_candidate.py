...
CONFIGURATION_DIRECTORY = REPOSITORY_ROOT / "configs" / "runtime" / "five_role_v2_candidate"


def test_five_role_runtime_v2_candidate_manifest_is_traceable_and_not_frozen() -> None:
    loader = RepositoryConfigurationLoader(REPOSITORY_ROOT, CONFIGURATION_DIRECTORY)
    manifest = loader.runtime_manifest

    assert manifest is not None
    assert manifest.configuration_set_id == "five-role-runtime-v2-development-candidate"
    assert manifest.configuration_status == "development_candidate_pending_review"
    assert manifest.user_approval_date is None
...
def test_five_role_runtime_v2_candidate_loads_all_versioned_strategies() -> None:
    loader = RepositoryConfigurationLoader(REPOSITORY_ROOT, CONFIGURATION_DIRECTORY)
    ...
    for job in jobs:
        ...
        assert configuration.l1_rules_configuration_version == "3.0.0"
        assert configuration.models_configuration_version == "3.0.0"
