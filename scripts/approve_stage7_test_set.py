...
SOURCE_DIRECTORY = Path("data/to_review/stage7_test_v1")
FROZEN_DIRECTORY = Path("data/frozen_test/stage7_v1")
REVIEWER_REFERENCES = ("reviewer-user-001", "reviewer-peer-001")
APPROVAL_STATEMENT = (
    "Tôi duyệt toàn bộ 50 case Stage 7 phiên bản 1.0.1, gồm 5 case đã sửa, và chấp "
    "nhận kết quả thảo luận chung của hai người là human review cuối cùng."
)
REVIEW_NOTE = (
    "Hai người đã thảo luận và thống nhất requirement status, năm nhóm điểm, nhãn và rationale "
    "cho toàn bộ 50 case Stage 7 phiên bản 1.0.1. Đây là review theo hội đồng đồng thuận, "
    "không phải hai lượt chấm độc lập."
)
...
def write_frozen_stage7_test_set(
    ...
    if not source_report.passed or source_report.warnings:
        raise ValueError("Stage 7 source dataset must pass QC without warnings")
    source_manifest_path = source_directory / "manifest.json"
    source_manifest = Stage7TestManifest.model_validate_json(
        source_manifest_path.read_text(encoding="utf-8")
    )
    if (
        source_manifest.dataset_id != "stage7-five-role-test-v1"
        or source_manifest.dataset_version != "1.0.1"
        or source_manifest.runtime_configuration_set_id != "five-role-runtime-v1"
    ):
        raise ValueError("legacy Stage 7 approval received an incompatible dataset manifest")
    profiles = cast(
    ...
    rubrics = cast(
        tuple[ScoringRubric, ...], _json_lines(source_directory / "rubrics.jsonl", ScoringRubric)
    )
    ...
    manifest = Stage7FrozenManifest(
        ...
        source_directory="data/to_review/stage7_test_v1",
    ...
    if not report.passed or report.warnings:
        raise ValueError("frozen Stage 7 dataset failed quality control")
...
def main() -> None:
    ...
    for path in write_frozen_stage7_test_set(
        REPOSITORY_ROOT,
