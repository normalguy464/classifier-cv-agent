...
Đây là prototype nghiên cứu của Classifier Agent trong hệ thống tuyển dụng có AI hỗ trợ. Phạm vi triển khai tám Stage đã hoàn thành ở mức phục vụ đồ án: FastAPI backend, Runtime v2 cho năm vai trò, giao diện Next.js, chế độ offline và LLM, human review cùng lịch sử quyết định.
...
- [Chế độ demo Stage 8](docs/stage_8_demo_modes.md): cách phân biệt offline và LLM .
...
- L3 đánh giá thông tin trong CV bằng adapter offline hoặc LLM.
...
| Internet | Cần khi cài dependency, tải embedding model lần đầu hoặc gọi LLM |
...
Nếu dùng OpenAI LLM, bổ sung hoặc cập nhật các biến sau trong `.env`:
...
Trên giao diện, chọn `LLM`, xác nhận khả năng phát sinh phí rồi mới chạy. Nếu thấy `fetch failed`, kiểm tra backend cổng `8001` và khởi động lại frontend sau khi thay `.env`. Hệ thống không tự fallback sang offline khi LLM lỗi.
...
Không cần gọi LLM để chạy test thường hoặc E2E. Adapter fake và mock được dùng để tránh chi phí và giữ kết quả tái lập.
...
- Người clone repository phải tự tạo `.env` và dùng API key của họ nếu muốn chạy LLM.
...
- LLM tạo rationale tự nhiên hơn nhưng không thay thế ground truth và không đảm bảo sửa được lỗi L1/L2.
